...
def run_task(name: str, passthrough: Sequence[str]) -> int:
    ...
    if commands and commands[0][0] == UV:
        _require_uv()
    if passthrough and len(commands) != 1:
        # Refused rather than guessed at. A multi-step task has no obvious place
        # to put extra arguments, and appending them to whichever step happens
        # to be last means `python tasks.py all -k something` silently passes a
        # pytest flag to the documentation builder.
        sys.exit(
            f"task {name!r} runs {len(commands)} commands, so there is nowhere "
            f"unambiguous to put {' '.join(passthrough)!r}.\n"
            "Pass extra arguments to a single-command task, for example "
            "'python tasks.py test -k <expression>'."
        )
