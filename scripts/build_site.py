...
REPOSITORY_URL: Final[str] = "https://github.com/kogunlowo123/llmops-observability"
...
PAGES: Final[tuple[Page, ...]] = (
    ...
    Page(Path("docs/windows.md"), "windows", "Telemetry windows", "Reference"),
    Page(Path("docs/objectives.md"), "objectives", "Objectives", "Reference"),
    Page(Path("docs/burnrate.md"), "burnrate", "Burn rate", "Reference"),
    Page(Path("docs/cost.md"), "cost", "Cost accounting", "Reference"),
    Page(Path("docs/cardinality.md"), "cardinality", "Cardinality", "Reference"),
...
def build(output: Path) -> int:
    ...
    for page in PAGES:
        ...
        (output / page.output_name).write_text(
            TEMPLATE.format(
                title=html.escape("llmops" if page.slug == "index" else f"{page.title} — llmops"),
